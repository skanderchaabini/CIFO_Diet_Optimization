...
# Calculate the mean fitness values for each fitness function
...
# Plot the line chart for each fitness function
for i, fitness_function in enumerate(fitness_functions):
    ax.plot(range(len(mean_fitness_values[i])), mean_fitness_values[i], label=fitness_function[1])
...
# Set the title
ax.set_title('Mean Best Fitness Progression for Different Fitness Functions')
